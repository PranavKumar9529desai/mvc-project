...
const CreateBatch = ({ farms, farm_id }: Props) => {
  ...
  useEffect(() => {
    if (farm_id) {
      setData("farm_id", farm_id.toString());
    }
  }, [farm_id]);
  ...
  return (
    <>
      <div className="mb-6">
        <Heading title="Add Batch" />
        <Breadcrumbs
          breadcrumbs={[
            { title: "Dashboard", href: route("dashboard") },
            { title: "Batches", href: route("batches.index") },
            { title: "Add Batch", href: route("batches.create") },
          ]}
        />
      </div>
...
CreateBatch.layout = (page: React.ReactNode) => <AppLayout children={page} />;
